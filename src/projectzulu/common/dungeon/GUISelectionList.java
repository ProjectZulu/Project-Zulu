...
//    private List<PairFullShortName<String,String>> listNames;
    ...
    public GUISelectionList(GuiLimitedMobSpawner parent, Node rootSoundNode, ListType listType, int listWidth, Point screenSize, Point backgroundSize) {
    	super(parent.getMinecraft(),
    			...
    			(screenSize.getY()-backgroundSize.getY())/2+25, (screenSize.getY()-backgroundSize.getY())/2+backgroundSize.getY()-20, // Top, Bottom, 
    	...
    	previous = new GuiButton(ButtonIDs.BACKWARDS.index, screenSize.getX()/2 +106, (screenSize.getY()+backgroundSize.getY())/2-240, 85, 20, "Parent Dir.");    	
    }
    ...
    protected void elementClicked(int clickedIndex, boolean var2){
    	...
    	if(childNode.numberOfChildren() > 0){
    		...
    		if(parent.getDataField(parent.currentDataField) instanceof CreatureFields){
    			((CreatureFields)parent.getDataField(parent.currentDataField)).setDataFromList(currentNode.getChild(clickedIndex).getFullName(), listType);
    			parent.closeList();
    		}
    	}
//    	if(parent.getDataField(parent.currentDataField) instanceof CreatureFields){
