...
//		this.targetTasks.addTask(4, new EntityAINearestAttackableTarget(this, EntityLiving.class, 16.0F, 0, false, true, IMob.mobSelector));
	...
	public boolean getCanSpawnHere() {
		...
		if (CustomEntityList.rhino.get().secondarySpawnRate - rand.nextInt(100) >= 0 && super.getCanSpawnHere()
				&& worldObj.canBlockSeeTheSky(var1, var2, var3)){
			wasSuccesful = true;
		}
		
		if(CustomEntityList.rhino.get().reportSpawningInLog){
			if(wasSuccesful){
	...
	protected String getLivingSound(){
		return "mods.sounds.rhinolivingsound";
	}
	...
	protected String getHurtSound(){ 
		return "mods.sounds.rhinohurtsound"; 
	}
	...
	protected void dropRareDrop(int par1) {
		if(Loader.isModLoaded(DefaultProps.BlocksModId) && ItemBlockList.mobHeads.isPresent()){
			entityDropItem(new ItemStack(ItemBlockList.mobHeads.get().blockID,1,14), 1);
		}
