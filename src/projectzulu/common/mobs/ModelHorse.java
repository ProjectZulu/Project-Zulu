...
public class ModelHorse extends ModelBase{
	...
	public void setLivingAnimations(EntityLiving par1EntityLiving, float par2, float par3, float par4) {
		...
		EntityHorseBase var5 = (EntityHorseBase)par1EntityLiving;
		
		/* Tail Rotation */
		NECKROT.rotateAngleX = (float)(45*Math.PI/180 + 7*Math.PI/180*MathHelper.cos(par2*0.6662F + (float)Math.PI) * 1.8F * zeroIfNegative(Math.log(par3+1)) );
		
		/* Tail Rotation */
		tail.rotateAngleZ = (float)( 0.5f*MathHelper.cos(par2*0.6662F + (float)Math.PI) * 1.8F * zeroIfNegative(Math.log(par3+1)) );
		...
		LEG2BOTROT.rotateAngleX = (float) Math.abs( MathHelper.cos(par2*0.6662F/2			   	 ) * 1.8F * zeroIfNegative(Math.log(par3+1)) );
		
		LEG3TOPROT.rotateAngleX = (float)( MathHelper.cos(par2*0.6662F + (float)Math.PI) * 1.8F * zeroIfNegative(Math.log(par3+1)) );
