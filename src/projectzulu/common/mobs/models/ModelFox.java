...
public class ModelFox extends ModelBase {
	...
	public void setLivingAnimations(EntityLiving par1EntityLiving, float par2, float par3, float par4) {
		...
		if(var5.getEntityState() == EntityStates.sitting){
			...
			LEG1ROT.rotateAngleX = (float)( MathHelper.cos(par2*0.6662F			   	 ) * 1.8F * zeroIfNegative(Math.log(par3+1)) );
			LEG1ROT.setRotationPoint(-1.5F, 2.5F, 7F);
			LEG2ROT.rotateAngleX = (float)( MathHelper.cos(par2*0.6662F + (float)Math.PI) * 1.8F * zeroIfNegative(Math.log(par3+1)) );
			LEG2ROT.setRotationPoint(1.5F, 2.5F, 7F);

			LEG3ROT.rotateAngleX = (float)( MathHelper.cos(par2*0.6662F			   	 ) * 1.8F * zeroIfNegative(Math.log(par3+1)) );
			LEG3ROT.setRotationPoint(-1.5F, 2.5F, 0F);
