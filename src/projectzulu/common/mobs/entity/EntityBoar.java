...
//		this.tasks.addTask(4, new EntityAIFollowOwner(this, this.moveSpeed,	10.0F, 2.0F));
		...
		this.tasks.addTask(6, new EntityAITempt(this, this.moveSpeed, Item.appleRed.itemID, false));
...
//		this.targetTasks.addTask(4, new EntityAINearestAttackableTarget(this, EntityLiving.class, 16.0F, 0, false, true, IMob.mobSelector));
	...
	public boolean getCanSpawnHere() {
		...
		if (CustomEntityList.BOAR.modData.get().secondarySpawnRate - rand.nextInt(100) >= 0 && super.getCanSpawnHere()){
			wasSuccesful = true;
		}
		
		if(CustomEntityList.BOAR.modData.get().reportSpawningInLog){
			if(wasSuccesful){
	...
	protected void dropFewItems(boolean par1, int par2){
		...
		for (int i = 0; i < var3; i++) {
			ItemStack loot = CustomEntityList.BOAR.modData.get().getLootItem(rand);
			if(loot != null){
	...
	protected void dropRareDrop(int par1) {
		if(Loader.isModLoaded(DefaultProps.BlocksModId) && BlockList.mobHeads.isPresent()){
			entityDropItem(new ItemStack(BlockList.mobHeads.get().blockID,1,7), 1);
		}
