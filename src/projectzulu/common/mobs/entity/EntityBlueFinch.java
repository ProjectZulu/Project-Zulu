...
public class EntityBlueFinch extends EntityFinch{
	...
	public boolean getCanSpawnHere() {
		...
		if (CustomEntityList.BLUEFINCH.modData.get().secondarySpawnRate - rand.nextInt(100) >= 0 && super.getCanSpawnHere() 
		...
		if(CustomEntityList.BLUEFINCH.modData.get().reportSpawningInLog){
			if(wasSuccesful){
	...
	protected void dropFewItems(boolean par1, int par2){
		...
		for (int i = 0; i < var3; i++) {
			ItemStack loot = CustomEntityList.BLUEFINCH.modData.get().getLootItem(rand);
			if(loot != null){
