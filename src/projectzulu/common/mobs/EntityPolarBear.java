...
public class EntityPolarBear extends EntityBear{
	...
	public String getTexture() {
		...
		this.texture = "/mods/bearpolar.png";
	...
	public boolean getCanSpawnHere() {
		...
		if (CustomEntityList.polarBear.get().secondarySpawnRate - rand.nextInt(100) >= 0 && super.getCanSpawnHere()
				&& worldObj.canBlockSeeTheSky(var1, var2, var3)){
			wasSuccesful = true;
		}
		
		if(CustomEntityList.polarBear.get().reportSpawningInLog){
			if(wasSuccesful){
	...
	protected void dropFewItems(boolean par1, int par2){
		...
		for (int i = 0; i < var3; i++) {
			ItemStack loot = CustomEntityList.polarBear.get().getLootItem(rand);
			if(loot != null){
	...
	protected void dropRareDrop(int par1) {
		if(Loader.isModLoaded(DefaultProps.BlocksModId) && ItemBlockList.mobHeads.isPresent()){
			entityDropItem(new ItemStack(ItemBlockList.mobHeads.get().blockID,1,5), 1);
		}
