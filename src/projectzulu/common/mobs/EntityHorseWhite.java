...
public class EntityHorseWhite extends EntityHorseBase{
	...
	public EntityHorseWhite(World par1World) {
		super(par1World);
	}
	...
	public boolean getCanSpawnHere() {
		...
		if (CustomEntityList.horseWhite.get().secondarySpawnRate - rand.nextInt(100) >= 0 && super.getCanSpawnHere() 
		...
		if(CustomEntityList.horseWhite.get().reportSpawningInLog){
			if(wasSuccesful){
	...
	protected void dropFewItems(boolean par1, int par2){
		...
		for (int i = 0; i < var3; i++) {
			ItemStack loot = CustomEntityList.horseWhite.get().getLootItem(rand);
			if(loot != null){
